...
[TestFixture]
public class LocationFeatureRepositoryTests
{
    private ILocationFeatureRepository _repository;
    private readonly IAppDbContext _context = Substitute.For<IAppDbContext>();

    [OneTimeSetUp]
    public void OneTimeSetup()
    {
        _repository = new LocationFeatureRepository(_context);
    }

    [Ignore("Pure delegation - no logic to test")]
    [Test]
    public Task GetByLocationIdAsync() => Task.CompletedTask;
